...
void * RtspServerListen(void*pParam)
{
    ...
    s32Rtn = listen(s32Socket, 50);   /*50,×??óμ?á??óêy*/
    if(s32Rtn < 0)
    {
    ...
    while ((s32CSocket = accept(s32Socket, (struct sockaddr*)&addrAccept, &nAddrLen)) >= 0)
    {
		...
		int nMaxBuf = 10 * 1024; // ?μí3???á·??? 2 x nMaxBuf μ??o3??óD?
...
/*****************************************************************
  Function:       è??úμ?oˉêy
  Description:   ó?ó?3ìDòè??úμ?
  Input:		argc,	2?êy??êy
  			argv,	2?êy×?·???????êy×é
  Return:        2ù×÷3é1?￡??ò·μ??0￡??T?ò·μ??<0      
